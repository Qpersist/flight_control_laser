...
float camera_time;
float wz_acc_temp,wz_acc1;
extern float acc_speed_arr[50 + 1];
extern u16 acc_cnt[2];
/////////////////////////////////////////////////////////////////////////////////////
...
//extern _hight_ctrl hight_PID_ctrl;
extern   float wz_speed_0,tempacc_lpf;
extern float mpu_fil_tmp[ITEMS];
...
/////////////////////////////////////////////////////////////////////////////////////
	...
	{
		f.send_version = 0;
		ANO_DT_Send_Version(7,000,100,400,0);
	}
/////////////////////////////////////////////////////////////////////////////////////
	else if(f.send_status)
	{
		f.send_status = 0;
		ANO_DT_Send_Status(Roll,-Pitch,Yaw,GY_530.relative_height,mode_state,fly_ready);	
	}	
/////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////
	else if(f.send_senser)
	{
		...
		ANO_DT_Send_Senser(mpu6050.Acc.x,mpu6050.Acc.y,mpu6050.Acc.z,
												...
												ak8975.Mag_Val.x,ak8975.Mag_Val.y,Switch.count_mode);
//		ANO_DT_Send_Senser(hight_PID_ctrl.shell.hight_pid_error,hight_PID_ctrl.shell.hight_pid_KP_out
//							,hight_PID_ctrl.shell.hight_pid_merrer,hight_PID_ctrl.shell.hight_pid_KI_out,
//											hight_PID_ctrl.shell.hight_pid_KD_out,hight_PID_ctrl.shell.hight_pid_KD_out,
//												thr_value,thr_value_old,mid_line);
//		ANO_DT_Send_Senser(CCD_CTRL_PID.error,CCD_CTRL_PID.PID_out,camera_Trance.error,camera_Trance.merrer,camera_Trance.devir,
//							camera_Trance.PID_out,camera_Trance.KP_out,camera_Trance.KD_out,mpu6050.Acc.x );
//		ANO_DT_Send_Senser(CCD_CTRL_PID.shell_Pout ,CCD_CTRL_PID_H.shell_Pout ,OF_DX2FIX, camera_Trance.take_off_flag,
//							OF_ALT2,hight_PID_ctrl.core.set_hight,
//							Duty.time,Duty.mode,camera_Trance.trance_flag);
//		ANO_DT_Send_Senser( Switch2.count_mode ,Switch2.flag ,change_Hight_data,mode_state,
//							change_CCD_data,camera_Trance.error_H,
//							camera_Trance.error_V,camera_Trance.take_off_flag,camera_Trance.trance_flag);
		
		
	}	
/////////////////////////////////////////////////////////////////////////////////////
	else if(f.send_senser2)															//高度信息
	{
		f.send_senser2 = 0;
//		ANO_DT_Send_Senser2(ultra.relative_height,hight_PID_ctrl.shell.set_hight);//原始数据
		ANO_DT_Send_Senser2(GY_530.status,GY_530.relative_height);//原始数据
	}	
/////////////////////////////////////////////////////////////////////////////////////
...
struct _camera_PID camera_PID_data;
extern u8 CCD_OK_flag;
void My_Data_Receive_Prepare(u8 data)
{
	static u8 RxBuffer[50];
	static u8 _data_len = 0,_data_cnt = 0;
	static u8 state = 0;
	
	if(state==0&&data==0x02)
	{
		state=1;
		RxBuffer[0]=data;
	}
	...
	else if(state==5)
	{
		state = 0;
		RxBuffer[4+_data_cnt]=data;
		MY_DT_Data_Receive_Anl(RxBuffer,_data_cnt+5);
	}
	else
		state = 0;
}

void MY_DT_Data_Receive_Anl(u8 *data_buf,u8 num)
{
	...
	if(!(*(data_buf)==0x02 && *(data_buf+1)==0xfe))		return;		//判断帧头
	
	
	if(*(data_buf+2)==0X01)
	{
		
		camera_Trance.data =  (vs16)(*(data_buf+4)<<8)|*(data_buf+5);
//        camera_Trance.data  = camera_Trance.error_H ;
		camera_Trance.data_V = (vs16)(*(data_buf+6)<<8)|*(data_buf+7);
//		camera_Trance.error_V = (vs16)(*(data_buf+6)<<8)|*(data_buf+7);

		camera_Trance.trance_flag =   (*(data_buf+8));
        camera_Trance.sound = (*(data_buf+9)) ;
		camera_Trance.take_off_flag = (*(data_buf+10));
		
		
		camera_Trance.devir_H = (camera_Trance.data - camera_Trance.error_H_old);
		camera_Trance.devir_V = (camera_Trance.data_V - camera_Trance.error_V_old);
		camera_Trance.error_H_old = camera_Trance.data   ;
		camera_Trance.error_V_old = camera_Trance.data_V ;
//		 feed_error(camera_Trance.data ,0,(int *)camera_Trance.error_H,0,OF_ALT2);
		
//		camera_Trance.error_H  =  camera_Trance.data  - OF_ALT2 * tan(Roll / 57.3f);
		
		if(camera_Trance.trance_flag ==0)
		{
			if(Switch.count_mode == 0)
				Duty.mode =0 ;
			else if(Switch.count_mode == 1)      			
				Duty.mode = 0;
			else if(Switch.count_mode ==2)
				Duty.mode = 2 ;
		}
//		if(camera_Trance.data != camera_Trance.data_old)
//		{
//			camera_Trance.trance_flag =  camera_Trance.data;
//		}
//        camera_Trance.data_old = camera_Trance.data;
...
//		}
	...
	if(*(data_buf+2)==0X02)
	{
		 camera_PID_data.Pitch  = 0.001*( (vs16)(*(data_buf+4)<<8)|*(data_buf+5) );
         camera_PID_data.ROll  = 0.001*( (vs16)(*(data_buf+6)<<8)|*(data_buf+7) );
         camera_PID_data.YAW  = 0.001*( (vs16)(*(data_buf+8)<<8)|*(data_buf+9) );
		 camera_PID_data.YAW_Error =(vs16)(*(data_buf+11)<<10)|*(data_buf+11);
	}
}
/////////////////////////////////////////////////////////////////////////////////////
