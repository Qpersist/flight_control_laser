...
//    direction_control(T,Switch.count_mode);                 //方向控制（手动和自动）
//=========================== 期望角度 ========================================
...
//	except_A.z  = MAX_CTRL_ANGLE  *( my_deathzoom( (CH_filter[YAW]) ,0,80 )/500.0f );  //30
	if( Thr_Low == 0 )
	{
...
//==============================================================================	
  ...
  /* 得到角度误差 */
	...
	ctrl_2.err.z =  To_180_degrees(  ctrl_angle_offset.y + except_A.z -Yaw	 );
	/* 计算角度误差权重 */
...
//	ctrl_1.out.z = 4 *( ctrl_1.FB *LIMIT((0.45f + 0.55f*ctrl_2.err_weight.z),0,1)*except_AS.z + ( 1 - ctrl_1.FB ) *ctrl_1.PID[PIDYAW].kp   *( ctrl_1.err.z + ctrl_1.err_d.z ) );
	//*(MAX_CTRL_ASPEED/300.0f);
	myself_Thr_Ctrl(T);// 油门控制
//	Thr_Ctrl(T);// 油门控制
...
void myself_Thr_Ctrl(float T)
{
	...
	hight_ctrl(hight_PID_ctrl.core.set_hight);                      //定高部分油门控制
		
	if( mode_state== 0 )
	{
		thr_value = 0;                                             //待机状态 最好改改，失控保护
		
	}
	else if(mode_state== 1)
	{
		thr_value  = LIMIT(thr,0,10 *MAX_THR *MAX_PWM/100);        //手动遥控

		thr_value_old = thr_value;
	}
	else if(mode_state== 2)
	{

		thr_value = thr_value_old +  hight_PID_ctrl.shell.hight_pid_out;	
		
		thr_value  = LIMIT(thr_value,0,800); 

	}
//	else if(mode_state== 2&&fly_ready)
//	{
//		
//		if(camera_Trance.take_off_flag ==1)                         //自动降落
//		{
//			thr_value_up++;
//			if(thr_value_up== 20)
//			{
//				hight_PID_ctrl.core.set_hight -- ;
//                if(hight_PID_ctrl.core.set_hight < OF_ALT2 - 10)
//				{
//					thr_value_old -= 3;
//				}
//				
...
//			thr_value = thr_value_old +  hight_PID_ctrl.shell.hight_pid_out ;
//		}
//		else if(change_Hight_data ==1)                                                   //一键起飞
//		{
//			thr_value_up++;
//			if(thr_value_up== 5)
//			{
//				thr_value ++; 
//				thr_value = LIMIT(thr_value,0,600);
//				thr_value_up= 0;
//			}
//			thr_value_old =  thr_value;
//		}
...
void All_Out(float out_roll,float out_pitch,float out_yaw)
{
  ...
  float curve[MAXMOTORS];
	...
	正负号需要根据实际情况改动
	先只用roll 左边电机与右边电机为两组，符号不同 向右偏 右边电机油门上升 依次类推
	
	
	*********************************/
	posture_value[0] = - out_roll + out_pitch + out_yaw + CCD_CTRL_PID.PID_out + CCD_CTRL_PID_H.PID_out;
	posture_value[1] = + out_roll + out_pitch - out_yaw - CCD_CTRL_PID.PID_out + CCD_CTRL_PID_H.PID_out;
	posture_value[2] = + out_roll - out_pitch + out_yaw - CCD_CTRL_PID.PID_out - CCD_CTRL_PID_H.PID_out;
	posture_value[3] = - out_roll - out_pitch - out_yaw + CCD_CTRL_PID.PID_out - CCD_CTRL_PID_H.PID_out;
